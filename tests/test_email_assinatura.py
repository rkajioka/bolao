"""Assinatura inline (base64) no corpo HTML de todos os e-mails."""
...
def _limpar_cache_assinatura():
    email_service._fragmento_assinatura_html.cache_clear()
    yield
    email_service._fragmento_assinatura_html.cache_clear()


def test_aplicar_assinatura_adiciona_base64_e_quebras(tmp_path, monkeypatch) -> None:
...
def test_enviar_email_outlook_aplica_assinatura_no_payload(monkeypatch) -> None:
    ...
    def fake_post(url, headers=None, json=None, timeout=None):  # noqa: ANN001
        captured["corpo_html"] = json["message"]["body"]["content"]
        response = type("R", (), {})()
    ...
    assert corpo in captured["corpo_html"]
    assert "data:image/png;base64," in captured["corpo_html"]
